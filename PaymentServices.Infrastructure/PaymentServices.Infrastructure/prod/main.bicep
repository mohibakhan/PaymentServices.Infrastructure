...
module planCompliance '../modules/appserviceplan.bicep' = {
  name: 'plan-compliance-${environment}'
  params: {
    planName: 'appcs-${prefix}-compliance-${environment}-${location}'
    location: location
    skuName: 'EP1'      // EP1 for now — upgrade to EP2 post-Alloy integration
    minimumInstances: 2
    maximumInstances: 20
    tags: tags
  }
...
module planEventNotification '../modules/appserviceplan.bicep' = {
  name: 'plan-evtnotif-${environment}'
  params: {
    planName: 'appcs-${prefix}-evtnotif-${environment}-${location}'
    location: location
    skuName: 'EP1'
    minimumInstances: 2
    maximumInstances: 20
...
module accountResolutionFunctionApp '../modules/functionapp.bicep' = {
  ...
  params: {
    ...
    managedIdentityClientId: managedIdentityClientId
    environment: toUpper(environment)
    serviceBusConnString: serviceBus.outputs.connectionString
    tags: tags
  }
}
...
module complianceFunctionApp '../modules/functionapp.bicep' = {
  ...
  params: {
    ...
    location: location
    appServicePlanId: planCompliance.outputs.planId
...
module eventNotificationFunctionApp '../modules/functionapp.bicep' = {
  ...
  params: {
    ...
    location: location
    appServicePlanId: planEventNotification.outputs.planId
