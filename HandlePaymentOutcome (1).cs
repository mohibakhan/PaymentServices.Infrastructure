...
public class HandlePaymentOutcome
{
    ...
    {
        var payment = (await _paymentCosmosDB.FindAllItemsAsync(outcome.EvolveId)).FirstOrDefault();
        if (payment is null)
        {
            _logger.LogError(
        ...
        // so there's nothing more to persist here.
        await _tabaPay.ProcessPayment(payment);
